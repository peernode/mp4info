...
filetime=sys.argv[1]
filename="./data/uncompress_fail_day_beta"
...
plt.plot(data["total"],"*-b",label=u"总解压次数")
plt.ylabel(u"总解压次数",fontproperties=zhfont1)
...
ax2=ax1.twinx()
plt.plot(data["fail"]/(data["total"]),"*-r",label=u"解压失败率")
plt.ylabel(u"解压失败率(100%)",fontproperties=zhfont1)
#plt.ylim(0,0.001)
#ax2.set_yticks(np.linspace(0, 0.001, 11))
plt.legend(loc="upper right",prop=zhfont1)
plt.grid()

plt.title(u"【Beta版本】解压压缩头失败率 by day",fontproperties=zhfont1)

plt.savefig("./png/uncompress_fail_ratio_day_beta_"+filetime)

print "draw"+filetime+" uncompress beta date png"
